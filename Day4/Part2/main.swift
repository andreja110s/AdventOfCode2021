...
var winningBoards = 0
var numberOfBoards = arrayOfBoards.count

for chosenNumber in chosenNumbers {
    
    for index in 0..<(numberOfBoards) {
        
        if arrayOfBoards[index].hasAlreadyWon {
            continue
        }
        ...
        if arrayOfBoards[index].HasColumnSelected() || arrayOfBoards[index].HasRowSelected() {
            ...
            arrayOfBoards[index].hasAlreadyWon = true
            
            winningBoards += 1
            
            if winningBoards == numberOfBoards {
